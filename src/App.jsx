...
import { Heart, Sparkles, Sun, Moon, Battery, CalendarHeart, Smile, Frown, Meh, BookOpen, Coffee, Briefcase, Edit2, Trash2, Search, Download, BarChart3, TrendingUp, Calendar, Dot } from 'lucide-react';
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      const newEntry = {
        id: Date.now(),
        date: selectedDate,
        mood, energy, tag, happiness, gratitude, growth, notes
      };
  ...
  return (
    <div className={`min-h-screen ${bgClass} font-sans selection:bg-pink-200 transition-colors duration-300 relative`}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[10%] left-[10%] w-32 h-32 rounded-full bg-pink-200/20 blur-3xl"></div>
        <div className="absolute top-[30%] right-[15%] w-40 h-40 rounded-full bg-yellow-200/20 blur-3xl"></div>
        <div className="absolute bottom-[20%] left-[20%] w-36 h-36 rounded-full bg-blue-200/20 blur-3xl"></div>
        <div className="absolute bottom-[10%] right-[10%] w-28 h-28 rounded-full bg-purple-200/20 blur-3xl"></div>
      </div>

      {/* 顶部导航 */}
      <header className={`${darkMode ? 'bg-gray-800/70 border-gray-700' : 'bg-white/70 border-pink-100'} backdrop-blur-md border-b shadow-sm sticky top-0 z-10 transition-colors`}>
                ...
                <div className="space-y-6 md:col-span-1">
                  ...
                  <div className={`${cardClass} rounded-3xl p-6 shadow-sm border transition-colors`}>
                    ...
                    <div className="flex gap-2 mb-3">
                      <button
                        onClick={goToday}
                        className={`flex-1 text-xs px-2 py-1.5 rounded-xl border transition-colors ${
                          darkMode 
                    ...
                    <div className="relative">
                      ...
                      {recordedDates.has(selectedDate) && (
                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                          <Dot className="text-pink-400 fill-pink-400" size={16} />
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-6 md:col-span-2">
                  ...
                  <div className={`${cardClass} rounded-3xl p-6 shadow-sm border transition-colors`}>
                    ...
                    <textarea 
                      ...
                      className={`w-full h-20 rounded-2xl p-4 focus:outline-none focus:ring-gray-300 resize-none ${inputClass} transition-colors`}
                    />
                  </div>
